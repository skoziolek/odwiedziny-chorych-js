...
**Ostatnia weryfikacja:** 2026-06-11  
**Wersja do wdrożenia:** **1.2.8** (tag Git: **`v1.2.8`**)  
...
W środku musi być m.in. plik **`odwiedziny-chorych.php`** (nagłówek wtyczki z **Version: 1.2.8**).
...
2. Przy **v1.2.8** pobierz załącznik **`odwiedziny-chorych-v1.2.8.zip`** (to jest sama wtyczka — nie wybieraj „Source code”, jeśli chcesz tylko paczkę do WordPressa).  
3. W WordPress: **Wtyczki → Dodaj nową → Wyślij wtyczkę na serwer** — wgraj ten ZIP.

**Jeśli przy v1.2.8 nie ma jeszcze załącznika:** osoba z uprawnieniami do repozytorium może go wygenerować: [Actions → Release WordPress plugin → Run workflow](https://github.com/skoziolek/odwiedziny-chorych-js/actions/workflows/release-wordpress.yml).

### Alternatywa: pobranie z tagu (ręczne wycinanie folderu)

1. Strona tagu: https://github.com/skoziolek/odwiedziny-chorych-js/releases/tag/v1.2.8 (lub zakładka **Tags** przy **`v1.2.8`**).
...
1. Przygotuj folder `odwiedziny-chorych` jak wyżej (z tagu **v1.2.8**).  
2. Wgraj do `wp-content/plugins/` (nadpisz pliki przy aktualizacji).  
3. W panelu WordPress: **Wtyczki** — upewnij się, że widać wersję **1.2.8**, w razie potrzeby włącz wtyczkę ponownie.  
...
- [ ] Pobrano kod z tagu **v1.2.8** (albo z gałęzi `feature/js-migration` w stanie zgodnym z tym tagiem).  
...
- [ ] Po wdrożeniu: wersja w panelu WP = **1.2.8**.

---

## Wersja (źródło prawdy)

- **Nazwa wersji / tag:** `v1.2.8`  
- **Numer w panelu WordPress:** **1.2.8** (pole Version w `odwiedziny-chorych.php`)
